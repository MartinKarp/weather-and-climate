...
def arg_parser():
    ...
    Basic: %(prog)s Downslope.npz\n\
    Example: %(prog)s -o plot -var horizontal_velocity -t 5 Downslope.npz"
    description = """
    ...
    # Optional arguments
    op.add_argument("-o", dest="figname", default='plot.pdf',
                    help="Name of the output figure",
                    metavar="FILE.pdf")
    op.add_argument("-v", dest="varname", default='horizontal_velocity',
                    metavar="VAR", help="Variable(s) to be plotted")
    op.add_argument("-t", dest="time", default='-1', metavar="timestep",
                    help="Output timestep. default=last", type=int)
    op.add_argument("-a", dest='anim', action='store_true',
                    help="interactive plot")
    op.add_argument("--zlim", dest="zlim", default=(0, 10), nargs=2,
                    metavar=("min", "max"), help="set the z-axis window [km]",
                    type=float)
    op.add_argument("--tci", dest="tci", default=2, metavar="interval", type=int,
                    help="set theta contouring interval [K]")
    op.add_argument("--tlim", dest="tlim", default=(0, 400),  nargs=2,
                    metavar=("min", "max"),
                    help="set limits for the theta range [K]", type=float)
    # horizontal_velocity
    op.add_argument("--vci", dest="vci", default=2, metavar="interval", type=int,
                    help="set velocity contouring interval [m/s]")
    op.add_argument("--vlim", dest="vlim", default=(float('nan'), float('nan')),  nargs=2,
                    metavar=("min", "max"), help="set limits for velocity range [m/s]",
                    type=float)
    # specific_humidity
    op.add_argument("--qvci", dest="qvci", default=0.25, metavar="interval",
                    type=float, help="set qv contouring interval [g/kg]")
    op.add_argument("--qvlim", dest="qvlim", default=(0.0, float('nan')),  nargs=2,
                    metavar=("min", "max"), help="set limits for the qv range [g/kg]",
                    type=float)
    # specific_cloud_liquid_water_content
    op.add_argument("--qcci", dest="qcci", default=0.1, metavar="interval",
                    type=float, help="set qc contouring interval [g/kg]")
    op.add_argument("--qclim", dest="qclim", default=(0.1, float('nan')),  nargs=2,
                    metavar=("min", "max"), help="set limits for the qc range [g/kg]",
                    type=float)
   # cloud_number_density
    op.add_argument("--ncci", dest="ncci", default=4*10**8, metavar="interval",
                    type=float, help="set nc contouring interval")
    op.add_argument("--nclim", dest="nclim", default=(10**8, float('nan')),
                    nargs=2, metavar=("min", "max"), type=float,
                    help="set range for the nc contours")
    # rain_number_density
    op.add_argument("--nrci", dest="nrci", default=5*10**2, type=float,
                    metavar="interval", help="set nr contouring interval")
    op.add_argument("--nrlim", dest="nrlim", default=(10**2, float('nan')),
                    nargs=2, metavar=("min", "max"), type=float,
                    help="set range for the nr contours")
    # specific_rain_water_content
    op.add_argument("--qrci", dest="qrci", default=0.005, metavar="interval",
                    type=float, help="set qr contouring interval [g/kg]")
    op.add_argument("--qrlim", dest="qrlim", default=(0.005, float('nan')),  nargs=2,
                    metavar=("min", "max"), help="set limits for the qr range [g/kg]",
                    type=float)
    op.add_argument("--totpreclim", dest="totpreclim", default=(0, float('nan')),
                    nargs=2, metavar=("min", "max"), type=float,
...
def update_im_back(i, data, ax, fig):
    ...
    timestep = timestep - 1
    if timestep < 0:
        timestep = data[data.keys()[0]].shape[0] + timestep
...
def forward_play(event):
    if timestep < len(var.time) - 1:
        ani = FuncAnimation(fig, update_im_forward,
                            frames=range(len(var.time) - 2 - timestep),
                            fargs=(var, ax, fig), interval=interval,
                            repeat=False)
        fig.canvas.draw()
...
def back_play(event):
    if timestep > 0:
        ani = FuncAnimation(fig, update_im_back, frames=range(timestep - 1),
                            fargs=(var, ax, fig), interval=interval,
                            repeat=False)
        fig.canvas.draw()
...
def update(val):
    global timestep
    timestep, = np.where(
        abs(var.time - slider.val) == np.min(abs(var.time - slider.val)))[0]
    plot_figure(varnames, var, timestep, False)
...
def plot_dict(args, var, varnames):
    ...
    if v in varnames:
        ...
        if np.isnan(vMaxInt):
            maxVel = scale * np.nanmax(var.horizontal_velocity)
    ...
    if v in varnames:
        ...
        if np.isnan(vMaxInt):
            maxQv = scale * np.nanmax(var.specific_humidity)
            qvciDiff = int((maxQv - vMinInt) / args.qvci + 1.)
            vMaxInt = vMinInt + qvciDiff * args.qvci
    ...
    if v in varnames:
        ...
        # according to the values in the data.
        if np.isnan(vMaxInt):
            maxQc = scale * np.nanmax(var.specific_cloud_liquid_water_content)
            qcciDiff = int((maxQc - vMinInt) / args.qcci + 1.)
            vMaxInt = vMinInt + qcciDiff * args.qcci
    ...
    if v in varnames:
        ...
        # according to the values in the data.
        if np.isnan(vMaxInt):
            maxNc = scale * np.nanmax(var.cloud_number_density)
            ncciDiff = int((maxNc - vMinInt) / args.ncci + 1.)
            vMaxInt = vMinInt + ncciDiff * args.ncci
    ...
    if v in varnames:
        ...
        # according to the values in the data.
        if np.isnan(vMaxInt):
            maxNr = scale * np.nanmax(var.rain_number_density)
            nrciDiff = int((maxNr - vMinInt) / args.nrci + 1.)
            vMaxInt = vMinInt + nrciDiff * args.nrci
    ...
    if v in varnames:
        ...
        # according to the values in the data.
        if np.isnan(vMaxInt):
            maxQr = scale * np.nanmax(var.specific_rain_water_content)
            qrciDiff = int((maxQr - vMinInt) / args.qrci + 1.)
            vMaxInt = vMinInt + qrciDiff * args.qrci
            if (vMaxInt - vMinInt) / args.qrci < 1.:
                vMaxInt += args.qrci
...
def plot_figure(varnames, var, timestep, plot_cbar):
    ...
    for varname in varnames:
        ...
        valRange = np.arange(pd[varname]['clev'][0],
                             pd[varname]['clev'][-1] + pd[varname]['ci'],
                             pd[varname]['ci'])
        ticks = np.arange(pd[varname]['clev'][0],
                          pd[varname]['clev'][-1] + pd[varname]['ci'],
                          pd[varname]['ci'])
        ...
        if varname == 'specific_rain_water_content':
            ...
            if np.isnan(vMaxInt):
                ...
                tpDiff = int((maxTp - vMinInt) / tpi + 1.)
...
def plot_setup(varnames, timestep):
    ...
    if not set(varnames) <= set(pd.keys()):
        msg = ("specified variable not supported, "
               "please check: {0} -h".format(sys.argv[0]))
        sys.exit(msg)
...
if __name__ == '__main__':
    ...
    if anim:
        ...
        bforward_play = widgets.Button(
            plt.axes([0.635, 0.06, 0.09, 0.065]), '>>')
        ...
        slider = widgets.Slider(plt.axes([0.235, 0.01, 0.59, 0.03]), 'Time',
                                var.time[0], var.time[-1], valinit=0.0)
